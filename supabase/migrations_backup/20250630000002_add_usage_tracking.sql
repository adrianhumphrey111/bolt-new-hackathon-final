-- Add file_size_bytes to videos table for storage tracking
ALTER TABLE videos 
ADD COLUMN file_size_bytes bigint DEFAULT 0,
ADD COLUMN processed_file_size_bytes bigint DEFAULT 0;
...
CREATE INDEX idx_videos_file_sizes ON videos(file_size_bytes, processed_file_size_bytes) WHERE file_size_bytes > 0;
...
CREATE INDEX idx_exports_render_id ON exports(render_id);
...
-- Create RLS policies for renders table
CREATE POLICY "Users can view their own renders" ON renders
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can create their own renders" ON renders
